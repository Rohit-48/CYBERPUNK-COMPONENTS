...
export const Navbar = () => {
    return (
       <div
            className="bg-white dark:bg-[#2b2d42] dark:text-white text-black mx-auto max-w-5xl px-2 sm:px-6 lg:px-8  border-2 rounded-lg mt-10  border-black font-family-electrolize"
            style={{
                ...
                <div className="flex items-center gap-10 mr-10 ">
                    {navItems.map((item) => (
                        <Link href={item.path} key={item.label}> 
                            <span className="text-black dark:text-white font-family-electrolize font-bold text-md hover:scale-110 transform transition-transform duration-300 inline-block">{item.label}</span>
                        </Link>
                    ))}
                    <ModeToggle/>
                </div>
